...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, k,
                 stride=1, dilation=1, padding_mode='zeros', deploy=False, use_se=True):
    ...
    def forward(self, inputs):
        ...
        if self.dw_bn_layer is None:
            ...
            id_out = self.dw_bn_layer(inputs)  # block输入和输出维度一样的话层开头接一个bn

        x_conv_3x3 = []
        for k_idx in range(self.k):
            x = getattr(self, f'dw_3x3_{k_idx}')(inputs)
            # print(x.shape)
            x_conv_3x3.append(x)  # block内接K个depth-wise_3x3
...
class MobileOneNet(nn.Module):
    def __init__(self, blocks, ks, channels, strides, width_muls, deploy=False):
        ...
        self.stage0 = nn.Sequential(
            nn.Conv2d(3, int(channels[0] * width_muls[0]), kernel_size=3, stride=2, padding=1, bias=False),
        ...
        for idx, block_num in enumerate(blocks[1:]):
            ...
            for b_idx in range(block_num):
                stride = strides[idx] if b_idx == 0 else 1  # 只有第一个block下采样
                '''构造该层的block模块，一层由很多个一样的block串联'''
                block = MobileOneBlock(in_channels, out_channels, ks[idx], stride, deploy=deploy)  # 第一個block负责维度变化和下采样
                in_channels = out_channels
                module.append(block)
            setattr(self, f"stage{idx}", nn.Sequential(*module))
        self.CA0 = CoordAtt(64, 64)
