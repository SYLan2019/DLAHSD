...
def print_map_summary(mean_ap,
    ...
    header = ['class', 'gts', 'dets', 'recall', 'precision','F1', 'ap']  # 打印precision标题
    for i in range(num_scales):
        if scale_ranges is not None:
            print_log(f'Scale range {scale_ranges[i]}', logger=logger)
        table_data = [header]
        for j in range(num_classes):
            F1[i, j] = 2.0*precisions[i, j]*recalls[i, j]/(precisions[i, j]+recalls[i, j])
            row_data = [
                label_names[j], num_gts[i, j], results[j]['num_dets'],
                f'{recalls[i, j]:.3f}', f'{precisions[i, j]:.3f}',f'{F1[i, j]:.3f}' ,f'{aps[i, j]:.3f}'  # 打印precision的值
            ]
